...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    web_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🌐 정부24+ 메인", url=f"{DOMAIN}/")
    ]])
    await update.message.reply_text(text, parse_mode='HTML',
                                    reply_markup=main_kb(uid))
    await update.message.reply_text(
        f"📱 <b>정부24+ 웹 서비스</b>\n{DOMAIN}/",
        parse_mode='HTML', reply_markup=web_kb
    )
...
async def id_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 인라인 버튼 (민증 링크 + 메인 링크)
    id_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🪪 민증 보기", url=web_link),
        InlineKeyboardButton("🌐 정부24+ 메인", url=main_link),
    ]])
    ...
    await update.message.reply_text(
        ...
        f"👤 이름: {name}\n🆔 주민번호: {ssn}\n🏠 주소: {address}\n"
        f"📅 발급일: {issue_date}\n📍 발급지역: {region}\n\n"
        f"🔗 <b>민증 링크:</b> {web_link}",
        parse_mode='HTML', reply_markup=id_kb
    )
    await update.message.reply_text(
        "📱 아래 버튼으로 바로 확인하세요!",
        reply_markup=main_kb(uid)
...
async def show_my_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text, parse_mode='HTML', reply_markup=id_kb)
...
async def edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("메뉴로 돌아갑니다.", reply_markup=main_kb(uid))
    context.user_data.clear()
    return ConversationHandler.END
