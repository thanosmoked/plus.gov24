...
def ensure_tables():
    ...
    # 기존 users 테이블에 필요한 컬럼들이 없으면 추가 (마이그레이션)
    columns_to_add = [
        ('query', 'TEXT'),
        ('expiredate', 'TEXT'),
        ('osname', 'TEXT')
...
async def on_ready():
    try:
        ...
        if TEST_GUILD:
            ...
            print(f"🔧 테스트 서버({TEST_GUILD_ID})에 {len(synced)}개 명령어 동기화 완료")
        else:
            # 글로벌 동기화 (모든 서버)
            synced = await bot.tree.sync()
            print(f"🔧 글로벌 {len(synced)}개 명령어 동기화 완료")
        
        # 추가: 봇이 속한 모든 길드에도 자동 동기화
        for guild in bot.guilds:
            try:
                await bot.tree.sync(guild=guild)
                print(f"✅ 길드 '{guild.name}' (ID: {guild.id})에 명령어 동기화 완료")
            except Exception as e:
                print(f"⚠️ 길드 '{guild.name}' 동기화 실패: {e}")
